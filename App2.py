...
def calculate_resume_score(jd, resume_skills):
...
#connection = pymysql.connect(host='localhost', user='root', password='Racerrb@1995')
connection = pymysql.connect(host='localhost', user='root', password= 'Racerrb@1995', db='cv')
cursor = connection.cursor()


def insert_data(name, email, res_score, timestamp, no_of_pages, reco_field, cand_level, skills, recommended_skills,
                courses):
    DB_table_name = 'kv_table1'
...
def run():
    ...
    img = Image.open(r'C:\Users\kvhem\Downloads\Smart_Resume_Analyser_App-master\Smart_Resume_Analyser_App-master\Logo\SRA_Logo.jpg')
    img = img.resize((250, 250))
    st.image(img)

    # Create the DB
    db_sql = """CREATE DATABASE IF NOT EXISTS kvt1;"""
    cursor.execute(db_sql)
    connection.select_db("kvt1")
    ...
    if choice == 'Normal User':
        ...
        if pdf_file is not None:
            ...
            if resume_data:
                ...
                #st.write(f'skills are {resume_skills}')
                ...
                rs=calculate_resume_score(job_description, resume_skills)
                ### Resume writing recommendation
                st.subheader("**Resume Tips & Ideas💡**")
                resume_score=0
                resume_score += rs
                st.write(f'Job description resume score is {resume_score}')
        ...
        if st.button('Login'):
            if ad_user == 'Hema' and ad_password == 'hema123':
                st.success("Welcome ")
                # Display Data
                ...
                #                                  'Recommended Course'])
                # st.dataframe(df)
                # st.markdown(get_table_download_link(df, 'Admin_Data.csv', 'Download Report'), unsafe_allow_html=True)
                # ## Admin Side Data
                # query = 'select * from kv_table;'
                # Fetch data with recommended skills and courses
                query = 'SELECT ID, Name, Email_ID, resume_score, Timestamp, Page_no, Predicted_Field, User_level, Actual_skills, Recommended_skills, Recommended_courses FROM kv_table1;'
